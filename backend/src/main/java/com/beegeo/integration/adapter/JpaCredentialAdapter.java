...
public class JpaCredentialAdapter implements CredentialPort {
    ...
    public void encryptAndSave(String accountId, String platformCode, String secretValue) {
        ...
        auditEventPublisher.publish("integration", "saveCredential", accountId, "system", true);
    }
    ...
    public void markExpired(String accountId) {
        var credential = publishCredentialRepository.findByAccountId(accountId);
        var account = publishAccountRepository.findByAccountId(accountId);
        credential.ifPresent(PublishCredentialEntity::markExpired);
        account.ifPresent(existing -> existing.updateStatus("EXPIRED"));
        if (credential.isPresent() || account.isPresent()) {
            auditEventPublisher.publish("integration", "expireCredential", accountId, "system", true);
        }
    }

...
export function AssetsPage(props: { currentRole: UserRoleCode }) {
  ...
  async function submitForm() {
    ...
    try {
      ...
      setEditingItem(null);
      setForm(emptyAssetForm);
      setNotice(editingItem ? "素材已更新" : "素材已添加");
      await refresh();
    } catch (exception) {
  ...
  const tagOptions = Array.from(new Set(items.map((item) => item.tag)));
  ...
  return (
    <div className="page-stack">
      <PageHeader title="素材库" description="管理品牌、活动、视频和表格素材，供创作与发布复用。" actionText="添加素材" onAction={openCreate} actionDisabled={!canWrite} actionTitle={canWrite ? undefined : deniedTitle} />
        ...
        <Modal
          ...
          footer={
            <>
              ...
              <button className="primary-button" type="submit" form="asset-form" disabled={saving || !canWrite} title={canWrite ? undefined : deniedTitle}>
                {saving ? "保存中" : "保存"}
              </button>
            </>
          }
